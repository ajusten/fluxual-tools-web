...
const routes = [
    { path: '', component: _home_home_component__WEBPACK_IMPORTED_MODULE_3__["HomeComponent"] },
    {
        path: 'tools',
        children: [
            { path: 'json-format', loadChildren: () => Promise.all(/*! import() | tools-json-format */[__webpack_require__.e("default~tools-json-format~tools-json-to-schema~tools-json-to-xml~tools-json-to-yaml"), __webpack_require__.e("tools-json-format")]).then(__webpack_require__.bind(null, /*! ./tools/json-format */ "./src/app/tools/json-format/index.ts")).then(m => m.JsonFormatModule) },
            // { path: 'json-path', loadChildren:() => import("./tools/json-path").then(m => m.JsonPathModule)},
            { path: 'json-to-schema', loadChildren: () => Promise.all(/*! import() | tools-json-to-schema */[__webpack_require__.e("default~tools-json-format~tools-json-to-schema~tools-json-to-xml~tools-json-to-yaml"), __webpack_require__.e("tools-json-to-schema")]).then(__webpack_require__.bind(null, /*! ./tools/json-to-schema */ "./src/app/tools/json-to-schema/index.ts")).then(m => m.JsonToSchemaModule) },
            { path: 'json-to-xml', loadChildren: () => Promise.all(/*! import() | tools-json-to-xml */[__webpack_require__.e("default~tools-json-format~tools-json-to-schema~tools-json-to-xml~tools-json-to-yaml"), __webpack_require__.e("tools-json-to-xml")]).then(__webpack_require__.bind(null, /*! ./tools/json-to-xml */ "./src/app/tools/json-to-xml/index.ts")).then(m => m.JsonToXmlModule) },
            { path: 'json-to-yaml', loadChildren: () => Promise.all(/*! import() | tools-json-to-yaml */[__webpack_require__.e("default~tools-json-format~tools-json-to-schema~tools-json-to-xml~tools-json-to-yaml"), __webpack_require__.e("tools-json-to-yaml")]).then(__webpack_require__.bind(null, /*! ./tools/json-to-yaml */ "./src/app/tools/json-to-yaml/index.ts")).then(m => m.JsonToYamlModule) }
        ]
    }
];
...
let MetadataService = class MetadataService {
    ...
    getTool(id) {
        return this.tools.filter(t => t.id == id)[0];
    }
    getTools() {
        return this.tools;
    }
    getFacetValueIcon(tag) {
        switch (tag) {
    ...
    getSearchFacets(search) {
        const facets = {};
        const searchTags = search.map(t => `${t.category}:${t.title}`);
        var tools = this.tools.filter(t => searchTags.every(s => t.tags.includes(s)));
        search.forEach(s => {
            let facet = facets[s.category];
            if (!facet) {
                facet = {
                    title: s.category,
                    tags: {}
            ...
            tool.tags.forEach(tag => {
                ...
                if (!facet.tags[subCategory]) {
                    facet.tags[subCategory] = {
                        title: subCategory,
                        icon: this.getFacetValueIcon(tag),
                        category: category,
                        selected: false
                    };
                }
        ...
        const searchFacets = Object.keys(facets)
            .sort()
            .map(k => {
            var facet = facets[k];
            facet.tags = Object.keys(facet.tags).map(t => facet.tags[t]).sort();
            return facet;
        });
...
let HomeComponent = class HomeComponent {
    ...
    onSearchChange() {
        [this.tools, this.facets] = this.metadata.getSearchFacets(this.filters);
    }
};
